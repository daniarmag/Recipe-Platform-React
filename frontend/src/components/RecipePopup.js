...
const RecipePopup = ({ title, imageSrc, description, ingredients, nutritionalValues, preparation, onClose }) => {
  ...
  return (
<div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border  bg-white p-4 rounded-md shadow-lg z-50 text-center w-10/12 max-w-md mx-auto sm:w-3/5 md:w-3/5 lg:w-1/2 xl:w-1/3 max-h-[22]  popup">


    <button
        onClick={onClose}
        className="absolute top-0 right-0  text-gray-800 text-2xl font-bold bg-transparent cursor-pointer"
      >
        &times; 
    </button>
