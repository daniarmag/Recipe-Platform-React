...
function ShoppingList({ selectedRecipes, onClose }) {
  ...
  return (
    <div
      className={`fixed top-1/2 left-1/2 transform  -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-md shadow-lg z-50 text-left max-w-sm w-4/5 sm:w-full max-h-[90vh]  `}
    >
      <button
        onClick={onClose}
        ...
        {Object.entries(shoppingList).map(([ingredient, amount]) => (
          ...
          >
            <div>
              ...
              <span
                className={`text-lg ${
                  amount ? "" : "line-through text-gray-500"
                }`}
              >
                {ingredient}
              </span>
            ...
            <div className="relative flex">
              <button
                type="button"
                onClick={() =>
                  handleAmountChange(ingredient, {
                    target: { value: Math.max(0, parseInt(amount) - 1) },
                  })
                }
              ...
              <button
                ...
                onClick={() =>
                  handleAmountChange(ingredient, {
                    target: { value: parseInt(amount) + 1 },
                  })
                }
